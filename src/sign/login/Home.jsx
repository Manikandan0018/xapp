...
export const Home = ({ notifi,setNoti }) => {

  const [openPost,setOpenPost] = useState(false)
  const [feedType, setFeedType] = useState('foryou');
  const [profileShow, setProfileShow] = useState(true);

  const [peopleAccount,setPeopleAccount] = useState([])
  ...
  return (
   ...
   <div onClick={()=>(
      setOpenPost(true)
      )} className="fixed cursor-pointer flex justify-center items-center bg-amber-400  w-full">
  <div className="fixed bottom-5 w-13 h-13 rounded-full flex items-center justify-center bg-blue-500 text-white  ">
    <div className="absolute inset-0 rounded-full border-4 border-blue-500 animate-pulse lg:shadow-[0] lg:animate-none shadow-[0_0_20px_10px_rgba(96,165,250,0.7)]"></div>
    ...
    <div className="lg:ml-80">
      {notifi && <NotificationPage notifi={notifi} setNoti={setNoti} />}
      ...
      {!notifi && profileShow && (
        <div className="flex flex-col border border-gray-200 w-full lg:w-[700px] lg:ml-[90px] px-3 sm:px-5">
          ...
          {feedType === 'foryou' && openPost && ( <CreatePost />)}
          <Posts  feedType={feedType} />
        </div>
      )}

      {!profileShow && <PeopleProfile peopleAccount={peopleAccount} setPeopleAccount={setPeopleAccount} profileShow={profileShow} setProfileShow={setProfileShow} />}
<div className="hidden lg:flex flex-col fixed top-0 right-0 w-[400px] p-5 h-screen overflow-y-auto">
  ...
  <SuggestUser peopleAccount={peopleAccount} setPeopleAccount={setPeopleAccount} setProfileShow={setProfileShow} profileShow={profileShow} />
